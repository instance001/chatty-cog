...
- the main UI works without ChattyCog
- save/load or normal workflow still works standalone
- missing ChattyCog bridge hooks do not break the app

Blocker examples:

- only works when launched by ChattyCog
- crashes if `CHATTYCOG_*` env vars are missing
- module depends on ChattyCog-managed files to function at all

## 2) Hosted behavior inside ChattyCog

Pass if:

- ChattyCog discovers the module correctly
...
- ChattyCog hosts the module but does not own its real state model
- the bridge is optional and lightweight
- removing the bridge would remove compatibility, not destroy the tool

Blocker examples:

- module stores its core data in ChattyCog-only files
- module assumes ChattyCog APIs are always present
- module cannot function outside the ChattyCog ecosystem anymore
...
- suspend handoff helps the orchestrator understand what happened
...
- leaving the tab gives ChattyCog no meaningful context when the bridge is supposed to exist
...
- there is one obvious folder to drop into `chattycog_gui/modules/`

...
- keep ChattyCog compatibility optional
- make install and update steps obvious for other users
- preserve the handoff surfaces that let the module participate in larger in-app workflow loops

## Packaging rule of thumb

Ship the module as if ChattyCog does not exist.

Then add the ChattyCog plug files as a thin compatibility layer:
...
If keeping those plug files makes the module easier to hand work to the next tool or orchestrator pass, the boundary is doing its job.
...
- when extracted, the user should get exactly one folder they can drop into `chattycog_gui/modules/`
...
1. Download the zip
2. Extract it
3. Copy the extracted module folder into `chattycog_gui/modules/`
4. In ChattyCog, use **Modules -> Rescan modules**
...
If ChattyCog hosting depends on a built executable, say so clearly in `README.md` and `USER_MANUAL.md`.
...
- confirm it works without ChattyCog
- confirm missing bridge env vars do not break it

### Hosted in ChattyCog

- copy the folder into `chattycog_gui/modules/`
- rescan modules
- open the module in a tab
- confirm the native UI or webview loads
- use the module
- switch away from the tab
- confirm ChattyCog gets a meaningful handoff if the bridge is enabled
...
- `README.md`
- `USER_MANUAL.md`
- `CHANGELOG.md` using `docs/CHANGELOG_TEMPLATE.md`
- release notes using `docs/MODULE_RELEASE_NOTES_TEMPLATE.md`
...
- drop it into ChattyCog if they want
- use it inside a longer loop with the orchestrator or sibling specialist modules
- remove the ChattyCog plug later without destroying the module itself

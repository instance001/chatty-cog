...
Purpose:
- Give ChattyCog (and future module runners) a consistent "how to talk to this department" document.
...
Important boundary:
- This template is for ChattyCog-facing modules.
- If you want an EDU-safe version for Chatty-EDU, treat that as a separate host target with its own review and adaptation pass.
...
Add preferences that help the orchestrator coordinate:
...
When the user leaves this module tab, ChattyCog logs a "suspend rundown" event.
...
- `bridge/status.json` reports the current handoff state to ChattyCog

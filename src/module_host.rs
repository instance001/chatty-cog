...
use module_host_support::{
    ModuleCommandSpec, ResolvedCommand, WebviewHostConfig, resolve_webview_host_binary,
    resolve_webview_target, run_command_to_completion,
};
...
impl ModuleHostState {
    ...
    fn launch_webview(&mut self, module_dir: &Path, spec: &ModuleVisualLoad) -> Result<(), String> {
        ...
        if !helper.is_file() {
            return Err(format!(
                "Webview host binary not found: {}. Rebuild ChattyCog so the helper binary exists.",
                helper.display()
            ));
        }
        ...
        let title = spec
            ...
            .unwrap_or_else(|| "ChattyCog Module".to_string());
    ...
    pub fn sync(
        ...
        let mut needs_repaint = self.build_rx.is_some();
...
const WEBVIEW_HOST_CONFIG: WebviewHostConfig<'static> = WebviewHostConfig {
    windows_binary_name: "chattycog_webview_host.exe",
...
fn attach_window(hwnd: isize, target: HostRect) -> Result<(), String> {
    ...
    use windows_sys::Win32::UI::WindowsAndMessaging::{
        GWL_EXSTYLE, GWL_STYLE, SW_SHOW, SWP_FRAMECHANGED, SWP_NOACTIVATE, SWP_NOOWNERZORDER,
        SWP_NOZORDER, SetParent, SetWindowLongPtrW, SetWindowPos, ShowWindow, WS_CAPTION, WS_CHILD,
        WS_EX_APPWINDOW, WS_MAXIMIZEBOX, WS_MINIMIZEBOX, WS_POPUP, WS_SYSMENU, WS_THICKFRAME,
        WS_VISIBLE,
    };

    let parent =
        find_chattycog_window().ok_or_else(|| "ChattyCog parent window not found.".to_string())?;
    unsafe {
...
#[cfg(target_os = "windows")]
fn find_chattycog_window() -> Option<isize> {
    use windows_sys::Win32::Foundation::HWND;
    use windows_sys::Win32::UI::WindowsAndMessaging::FindWindowW;

    let title: Vec<u16> = "ChattyCog"
        .encode_utf16()

...
impl EcgBackend {
    fn new(payload: &mut EcgWindowPayload) -> Self {
        ...
        {
            payload.note =
                "System counters are not available on this OS. Monitoring ChattyCog activity."
                    .to_string();
        }
        ...
        if payload.note.is_empty() {
            payload.note =
                "System counters were unavailable. Monitoring ChattyCog activity instead."
                    .to_string();
        }
    ...
    fn sample_once(&mut self, payload: &mut EcgWindowPayload, max_history: usize) {
        ...
        if value >= 1.0 {
            if self.note_hold_samples > 0 {
                ...
                payload.note = "Recent ChattyCog workload activity is high.".to_string();
            } else {
                payload.note = "Recent ChattyCog workload activity is steady.".to_string();
            }
        } else {
            payload.note = "ChattyCog is idle right now.".to_string();
...
#[cfg(windows)]
mod windows_backend {
    use super::{EcgWindowPayload, push_history};
    use std::iter;
    ...
    fn add_counter(query: isize, path: &str) -> Result<isize, String> {
    ...
    fn read_single_counter_percent(counter: isize) -> Result<Option<f32>, String> {
        let mut value = unsafe { std::mem::zeroed::<PDH_FMT_COUNTERVALUE>() };
        let status = unsafe {
    ...
    fn read_gpu_percent(counter: isize) -> Result<Option<f32>, String> {
        ...
        let mut items = vec![
            unsafe { std::mem::zeroed::<PDH_FMT_COUNTERVALUE_ITEM_W>() };
        ...
        let busiest = items
            ...
            .fold(None::<f32>, |current: Option<f32>, value| {
                Some(current.map_or(value, |best: f32| best.max(value)))
            });

    ...
    fn send_discover(&mut self) {
        ...
        if let Ok(bytes) = serde_json::to_vec(&packet) {
            ...
            self.snapshot.status = if self.snapshot.available_for_connectivity {
                "Hosting on the LAN and scanning for nearby ChattyCog instances.".to_string()
            } else {
                "Scanning the local LAN for available ChattyCog instances.".to_string()
            };
            self.dirty = true;
        }
    }
    ...
    fn handle_connection_line(&mut self, connection_id: &str, address: SocketAddr, bytes: &[u8]) {
        ...
        if packet.kind == "handoff" {
            if let Some(handoff) = packet.handoff {
                ...
                self.snapshot.status =
                    "Received a handoff from another ChattyCog instance.".to_string();
            }
            ...
            if let Some(artifact) = packet.artifact {
                ...
                if self.seen_artifact_ids.contains_key(&artifact.artifact_id) {
                    ...
                    self.snapshot.status =
                        "Ignored a duplicate shared transfer from another ChattyCog instance."
                            .to_string();
                    self.dirty = true;
                    return;
                }
                ...
                match self.ingest_artifact_chunk(
                    ...
                    Ok(Some(received)) => {
                        ...
                        self.snapshot.status =
                            "Received a shared transfer from another ChattyCog instance."
                                .to_string();
                    }
                    Ok(None) => {}
                    Err(error) => {
                ...
                match event.validate() {
                    Ok(()) => {
                        if !self.seen_session_event_ids.contains_key(&event.event_id) {
                            ...
                            self.snapshot.status =
                                "Received a session event from another ChattyCog instance."
                                    .to_string();
                        }
                    }
                    Err(error) => {
...
fn make_device_name() -> String {
    ...
    format!("ChattyCog on {} ({})", host, std::process::id())
}
...
fn make_device_id() -> String {
    ...
    let now = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_millis();
    format!("chattycog-{host}-{}-{now}", std::process::id())
}

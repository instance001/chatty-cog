...
use chattycog_gui::module_bridge::{
    ModuleBridgeOutgoingRoomEvent, ModuleBridgeSharedRoomEvents, ModuleBridgeSharedRoomState,
    ModuleBridgeSharedState, ModuleBridgeStatus, append_bridge_outgoing_room_event,
    clear_bridge_shared_state, clear_bridge_status, read_bridge_incoming_assets,
    read_bridge_incoming_shared_state, read_bridge_shared_room_events,
...
const MODULE_FILE_SCHEME: &str = "chattycogmodule";
        ...
        .with_ipc_handler(move |req| {
            if let Some(module_dir) = bridge_dir.as_deref() {
                if let Err(err) = handle_ipc_message(module_dir, req.body().clone()) {
                    eprintln!("chattycog webview bridge error: {err:#}");
                }
            }
        });
...
fn parse_args() -> Result<Args> {
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--module-dir" => {
                module_dir = iter.next().map(std::path::PathBuf::from);
            }
            "--help" | "-h" => {
                print_help();
                std::process::exit(0);
    ...
    let title = title
        ...
        .unwrap_or_else(|| "ChattyCog Webview".to_string());
    let url = url
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
...
fn print_help() {
    eprintln!(
        "chattycog_webview_host --title <window title> --url <http(s)://...|file://...> [--module-dir <path>]"
    );
}
...
fn build_init_script(module_dir: Option<&std::path::Path>) -> String {
  ...
  const bridge = {{
    ...
    updateStatus(payload) {{
      ...
      try {{
        ...
        console.warn("chattyCogBridge.updateStatus failed", err);
        return false;
      }}
    }},
    clearStatus() {{
      ...
      try {{
        ...
        console.warn("chattyCogBridge.clearStatus failed", err);
        return false;
      }}
    }},
    updateSharedState(payload) {{
      ...
      try {{
        ...
        console.warn("chattyCogBridge.updateSharedState failed", err);
        return false;
      }}
    }},
    clearSharedState() {{
      ...
      try {{
        ...
        console.warn("chattyCogBridge.clearSharedState failed", err);
        return false;
      }}
    }},
    async readIncomingSharedState() {{
      ...
      try {{
        const response = await fetch("/__chattycog_bridge__/incoming_shared_state.json?ts=" + Date.now(), {{ cache: "no-store" }});
        if (!response.ok) return null;
        return await response.json();
      }} catch (err) {{
        console.warn("chattyCogBridge.readIncomingSharedState failed", err);
        return null;
      }}
    }},
    async readSharedRoomState() {{
      if (!bridge.available) return null;
      try {{
        const response = await fetch("/__chattycog_bridge__/shared_room_state.json?ts=" + Date.now(), {{ cache: "no-store" }});
        if (!response.ok) return null;
        return await response.json();
      }} catch (err) {{
        console.warn("chattyCogBridge.readSharedRoomState failed", err);
        return null;
      }}
    }},
    async readSharedRoomEvents() {{
      if (!bridge.available) return null;
      try {{
        const response = await fetch("/__chattycog_bridge__/shared_room_events.json?ts=" + Date.now(), {{ cache: "no-store" }});
        if (!response.ok) return null;
        return await response.json();
      }} catch (err) {{
        console.warn("chattyCogBridge.readSharedRoomEvents failed", err);
        return null;
      }}
    }},
    emitRoomEvent(payload) {{
      ...
      try {{
        ...
        console.warn("chattyCogBridge.emitRoomEvent failed", err);
        return false;
      }}
    }},
    async readIncomingAssets(laneId) {{
      ...
      try {{
        const response = await fetch("/__chattycog_bridge__/incoming_assets.json?lane=" + encodeURIComponent(laneId) + "&ts=" + Date.now(), {{ cache: "no-store" }});
        if (!response.ok) return [];
        const payload = await response.json();
        return Array.isArray(payload) ? payload : [];
      }} catch (err) {{
        console.warn("chattyCogBridge.readIncomingAssets failed", err);
        return [];
      }}
    }},
    incomingAssetUrl(laneId, payloadFileName) {{
      if (!bridge.available || !laneId || !payloadFileName) return "";
      return "/__chattycog_bridge__/incoming_assets/" + encodeURIComponent(laneId) + "/" + encodeURIComponent(payloadFileName);
    }},
    consumeIncomingAsset(laneId, assetId) {{
      ...
      try {{
        ...
        console.warn("chattyCogBridge.consumeIncomingAsset failed", err);
        return false;
      }}
    }}
  }};
  window.chattyCogBridge = Object.assign({{}}, window.chattyCogBridge || {{}}, bridge);
}})();
"#
    )
}
...
fn handle_ipc_message(module_dir: &std::path::Path, msg: String) -> Result<()> {
...
fn try_serve_module_request(
    ...
    if request_path == PathBuf::from("__chattycog_bridge__/incoming_shared_state.json") {
    ...
    if request_path == PathBuf::from("__chattycog_bridge__/shared_room_state.json") {
    ...
    if request_path == PathBuf::from("__chattycog_bridge__/shared_room_events.json") {
    ...
    if request_path == PathBuf::from("__chattycog_bridge__/incoming_assets.json") {
        let lane_id = request
            .uri()
            .query()
    ...
    if request_path
        ...
        .collect::<Vec<_>>()
        .starts_with(&[
            "__chattycog_bridge__".to_string(),
        ...
        ])
    {
        ...
        if parts.len() >= 4 {
            ...
            {
                let payload_path =
                    chattycog_gui::module_bridge::bridge_incoming_asset_lane_dir(root_dir, lane_id)

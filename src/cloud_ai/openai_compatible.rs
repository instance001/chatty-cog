...
impl CloudProviderAdapter for OpenAiCompatibleAdapter {
    ...
    fn chat_completion(
        ...
        extract_chat_completion_text(&value).with_context(|| {
            format!(
                "cloud model '{}' returned an unexpected chat response shape",
                self.target.display_name
            )
        })
    }
    ...
    fn embed_text(&self, input: &str) -> Result<Vec<f32>> {
        ...
        extract_embedding(&value).with_context(|| {
            format!(
                "cloud model '{}' returned an unexpected embeddings response shape",
                self.target.display_name
            )
        })
    }

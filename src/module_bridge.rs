...
impl ModuleBridgeStatus {
    ...
    pub fn payload_json_string(&self) -> Option<String> {
...
pub fn bridge_env(module_dir: &Path) -> Result<HashMap<String, String>> {
    ...
    env.insert("CHATTYCOG_HOSTED".to_string(), "1".to_string());
    env.insert(
        "CHATTYCOG_MODULE_DIR".to_string(),
        module_dir.display().to_string(),
    );
    env.insert(
        "CHATTYCOG_BRIDGE_DIR".to_string(),
        dir.display().to_string(),
    );
    env.insert(
        "CHATTYCOG_BRIDGE_STATUS".to_string(),
        status.display().to_string(),
    );
    env.insert(
        "CHATTYCOG_BRIDGE_LOG_SOURCES".to_string(),
        dir.join(BRIDGE_LOG_SOURCES_FILE).display().to_string(),
    );
    env.insert(
        "CHATTYCOG_BRIDGE_SHARED_STATE".to_string(),
        dir.join(BRIDGE_SHARED_STATE_FILE).display().to_string(),
    );
    env.insert(
        "CHATTYCOG_BRIDGE_INCOMING_SHARED_STATE".to_string(),
        dir.join(BRIDGE_INCOMING_SHARED_STATE_FILE)
            .display()
            .to_string(),
    );
    env.insert(
        "CHATTYCOG_BRIDGE_SHARED_ROOM_STATE".to_string(),
        dir.join(BRIDGE_SHARED_ROOM_STATE_FILE)
            .display()
            .to_string(),
    );
    env.insert(
        "CHATTYCOG_BRIDGE_SHARED_ROOM_EVENTS".to_string(),
        dir.join(BRIDGE_SHARED_ROOM_EVENTS_FILE)
            .display()
            .to_string(),
    );
    env.insert(
        "CHATTYCOG_BRIDGE_OUTGOING_ROOM_EVENTS".to_string(),
        dir.join(BRIDGE_OUTGOING_ROOM_EVENTS_FILE)
            .display()
            .to_string(),
    );
    env.insert(
        "CHATTYCOG_BRIDGE_INCOMING_ASSETS_DIR".to_string(),
